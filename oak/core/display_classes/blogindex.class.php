...
class Display_BlogIndex implements Display {
 ...
 * argument.
 * 
 * @throws Display_BlogIndexException
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_BlogIndexException("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_BlogIndexException("Input for parameter page is expected to be an array");
	}
...
{
	return new Display_BlogIndex($project, $page);
}
...
{
	return "oak:blog_index.".OAK_CURRENT_PAGE;
}
...
public function getLocationSelf ()
{
	...
	return str_replace($patterns, $replacements, $this->base->_conf['urls']['blog_index']);
}
...
class Display_BlogIndexException extends Exception { }
