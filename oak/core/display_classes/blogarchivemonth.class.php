...
public function __construct($project_info, $page_info)
{
	...
	if (!is_array($project_info)) {
		throw new Display_BlogArchiveMonthException("Input for parameter project_info is expected to be an array");
	}
	if (!is_array($page_info)) {
		throw new Display_BlogArchiveMonthException("Input for parameter page_info is expected to be an array");
	}
...
{
	return new Display_BlogArchiveMonth($project_info, $page_info);
}
...
public function getLocationSelf ()
{
	if ($this->_page_info['index_page']) {
		...
		return sprintf("index.php?page=%u&action=ArchiveMonth", $this->_page_info['id']);
	}
}
