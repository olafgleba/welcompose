...
public function __construct($project_info, $page_info)
{
	...
	if (!is_array($project_info)) {
		throw new Display_BlogArchiveMonth("Input for parameter project_info is expected to be an array");
	}
	if (!is_array($page_info)) {
		throw new Display_BlogArchiveMonth("Input for parameter page_info is expected to be an array");
	}
...
public function getRedirectLocationSelf ()
{
	if ($this->_page['index_page']) {
		return sprintf("Location: index.php?page=%u&action=ArchiveMonth", $this->_page['id']);
	} else {
		return 'index.php';
	}
}
