...
/**
 * Project: Oak
 * File: blogpodcastitms.class.php
 * 
...
class Display_BlogPodcastItms implements Display {
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_BlogPodcastItmsException("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_BlogPodcastItmsException("Input for parameter page is expected to be an array");
	}
...
{
	return new Display_BlogPodcastItms($project, $page);
}
...
{
	return "oak:blog_podcast_itms.".OAK_CURRENT_PAGE;
}
...
public function getLocationSelf ()
{
	...
	return str_replace($patterns, $replacements, $this->base->_conf['urls']['blog_podcast_itms']);
}
...
class Display_BlogPodcastItmsException extends Exception { }
