...
try {
	...
	$FORM->addRule('name', gettext('A text macro with the given name already exists'), 'testForNameUniqueness');

	// textfield for internal_name
	$FORM->addElement('text', 'internal_name', gettext('Internal name'), 
		array('id' => 'text_macro_internal_name', 'maxlength' => 255, 'class' => 'w300'));
	...
	$FORM->addRule('internal_name', gettext('Please enter a valid internal name'), 'regex',
		OAK_REGEX_TEXT_CONVERTER_INTERNAL_NAME);
	...
	// submit button
	$FORM->addElement('submit', 'submit', gettext('Add text macro'),
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('application/textmacros_add.html', OAK_TEMPLATE_KEY);
		...
		try {
			...
			// execute operation
			$TEXTMACRO->addTextMacro($sqlData);
		...
		// redirect
		header("Location: textmacros_add.php");
		exit;
	}
} catch (Exception $e) {
	// clean the buffer
