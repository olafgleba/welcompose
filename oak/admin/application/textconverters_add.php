...
try {
	...
	$FORM->addElement('text', 'internal_name', gettext('Internal name'), 
		array('id' => 'text_converter_internal_name', 'maxlength' => 255, 'class' => 'w300'));
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('application/textconverters_add.html', OAK_TEMPLATE_KEY);
		...
		// redirect
		header("Location: textconverters_add.php");
		exit;
	}
} catch (Exception $e) {
	// clean the buffer
