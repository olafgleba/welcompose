...
try {
	...
	// get box
	$box = $BOX->selectBox(Base_Cnc::filterRequest($_REQUEST['id'], OAK_REGEX_NUMERIC));
	...
	// submit button
	$FORM->addElement('submit', 'submit', gettext('Update box'),
		array('class' => 'submitbut140'));
	
	// set defaults
	$FORM->setDefaults(array(
		'page' => Base_Cnc::ifsetor($page['id'], null),
		'id' => Base_Cnc::ifsetor($box['id'], null),
	...
	if (!$FORM->validate()) {
		...
		try {
			...
			// execute operation
			$BOX->updateBox($FORM->exportValue('id'), $sqlData);
