...
try {
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('content/pages_blogs_pingserviceconfigurations_add.html', OAK_TEMPLATE_KEY);
		...
		// redirect
		header("Location: pages_blogs_pingserviceconfigurations_add.php?page=".$FORM->exportValue('page'));
		exit;
	}
} catch (Exception $e) {
	// clean the buffer
