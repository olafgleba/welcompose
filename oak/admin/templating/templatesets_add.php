...
try {
	...
	$FORM->addElement('text', 'name', gettext('Name'), 
		array('id' => 'template_set_name', 'maxlength' => 255, 'class' => 'w300'));
	$FORM->applyFilter('name', 'trim');
	$FORM->applyFilter('name', 'strip_tags');
	$FORM->addRule('name', gettext('Please enter a name'), 'required');
	$FORM->addRule('name', gettext('Please enter a valid name'), 'regex', OAK_REGEX_TEMPLATE_SET_NAME);
	$FORM->addRule('name', gettext('A template set with the given name already exists'), 'testForNameUniqueness');
	...
	// submit button
	$FORM->addElement('submit', 'submit', gettext('Add template set'),
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('templating/templatesets_add.html', OAK_TEMPLATE_KEY);
		...
		try {
			...
			// execute operation
			$TEMPLATESET->addTemplateSet($sqlData);
		...
		// redirect
		header("Location: templatesets_add.php");
		exit;
	}
} catch (Exception $e) {
	// clean the buffer
