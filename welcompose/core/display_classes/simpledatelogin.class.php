...
/**
 * Project: Welcompose
 * File: simpledatelogin.class.php
 * 
 ...
 * @return object
 */
function Display_SimpleDateLogin ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_SimpleDateLogin($args[0], $args[1]);
}

class Display_SimpleDateLogin extends Display_SystemLogin {
 ...
 * argument.
 * 
 * @throws Display_SimpleDateLoginException
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_SimpleDateLoginException("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_SimpleDateLoginException("Input for parameter page is expected to be an array");
	}
...
class Display_SimpleDateLoginException extends Exception { }
