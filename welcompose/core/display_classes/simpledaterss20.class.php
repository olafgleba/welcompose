...
/**
 * Project: Welcompose
 * File: simpledaterss20.class.php
 * 
 ...
 * @return object
 */
function Display_SimpleDateRss20 ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_SimpleDateRss20($args[0], $args[1]);
}

class Display_SimpleDateRss20 implements Display {
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_SimpleDateRss20Exception("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_SimpleDateRss20Exception("Input for parameter page is expected to be an array");
	}
...
public function getMainTemplateName ()
{
	...
	return "wcom:simple_date_rss20.".WCOM_CURRENT_PAGE;
}
...
class Display_SimpleDateRss20Exception extends Exception { }
