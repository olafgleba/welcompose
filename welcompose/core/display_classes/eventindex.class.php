 ...
 * @return object
 */
function Display_EventIndex ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_EventIndex($args[0], $args[1]);
}

class Display_EventIndex implements Display {
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_EventIndexException("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_EventIndexException("Input for parameter page is expected to be an array");
	}
...
{
	return "wcom:event_index.".WCOM_CURRENT_PAGE;
}
...
class Display_EventIndexException extends Exception { }
