 ...
 * @return object
 */
function Display_BlogCommentsRss20 ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_BlogCommentsRss20($args[0], $args[1]);
}

class Display_BlogCommentsRss20 implements Display {
 ...
 * argument.
 * 
 * @throws Display_BlogCommentsRss20Exception
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_BlogCommentsRss20Exception("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_BlogCommentsRss20Exception("Input for parameter page is expected to be an array");
	}
...
public function getMainTemplateName ()
{
	...
	return "wcom:blog_comments_rss20.".WCOM_CURRENT_PAGE;
}
...
class Display_BlogCommentsRss20Exception extends Exception { }
