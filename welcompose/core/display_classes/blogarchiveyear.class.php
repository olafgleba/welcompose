...
/**
 * Project: Welcompose
 * File: blogarchiveyear.class.php
 * 
 ...
 * @return object
 */
function Display_BlogArchiveYear ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_BlogArchiveYear($args[0], $args[1]);
}

class Display_BlogArchiveYear implements Display {
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_BlogArchiveYearException("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_BlogArchiveYearException("Input for parameter page is expected to be an array");
	}
...
{
	return "wcom:blog_archive_year.".WCOM_CURRENT_PAGE;
}
...
class Display_BlogArchiveYearException extends Exception { }
