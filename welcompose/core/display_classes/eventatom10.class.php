 ...
 * @return object
 */
function Display_EventAtom10 ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_EventAtom10($args[0], $args[1]);
}

class Display_EventAtom10 implements Display {
 ...
 * argument.
 * 
 * @throws Display_EventAtom10Exception
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_EventAtom10Exception("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_EventAtom10Exception("Input for parameter page is expected to be an array");
	}
...
public function getMainTemplateName ()
{
	...
	return "wcom:event_atom10.".WCOM_CURRENT_PAGE;
}
...
class Display_EventAtom10Exception extends Exception { }
