...
if (!class_exists('Display_SystemLogin'))Event {
	$path_parts = array(
		dirname(__FILE__),
 ...
 * @return object
 */
function Display_EventLogin ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_EventLogin($args[0], $args[1]);
}

class Display_EventLogin extends Display_SystemLogin {
 ...
 * argument.
 * 
 * @throws Display_EventLoginException
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_EventLoginException("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_EventLoginException("Input for parameter page is expected to be an array");
	}
...
class Display_EventLoginException extends Exception { }
