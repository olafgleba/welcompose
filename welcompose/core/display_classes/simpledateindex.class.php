...
/**
 * Project: Welcompose
 * File: simpledateindex.class.php
 * 
 ...
 * @return object
 */
function Display_SimpleDateIndex ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_SimpleDateIndex($args[0], $args[1]);
}

class Display_SimpleDateIndex implements Display {
 ...
 * argument.
 * 
 * @throws Display_SimpleDateIndexException
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_SimpleDateIndexException("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_SimpleDateIndexException("Input for parameter page is expected to be an array");
	}
...
{
	return "wcom:simple_date_index.".WCOM_CURRENT_PAGE;
}
...
class Display_SimpleDateIndexException extends Exception { }
