 ...
 * @return object
 */
function Display_EventRss20 ($args)
{
	// check input
	if (!is_array($args)) {
	...
	return new Display_EventRss20($args[0], $args[1]);
}

class Display_EventRss20 implements Display {
 ...
 * argument.
 * 
 * @throws Display_EventRss20Exception
...
public function __construct($project, $page)
{
	...
	// input check
	if (!is_array($project)) {
		throw new Display_EventRss20Exception("Input for parameter project is expected to be an array");
	}
	if (!is_array($page)) {
		throw new Display_EventRss20Exception("Input for parameter page is expected to be an array");
	}
...
public function getMainTemplateName ()
{
	...
	return "wcom:event_rss20.".WCOM_CURRENT_PAGE;
}
...
class Display_EventRss20Exception extends Exception { }
