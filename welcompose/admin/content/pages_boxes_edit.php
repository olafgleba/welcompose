...
try {
	...
	$name->addRule('callback', gettext('A box with the given name already exists'), 
		array(
			...
			'arguments' => array(array('page' => $page_id->getValue(), 'id' => $id->getValue()))
		)
	);	
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('content/pages_boxes_edit.html', WCOM_TEMPLATE_KEY);
		...
		try {
			...
			// execute operation
			$BOX->updateBox($id->getValue(), $sqlData);
		...
		// redirect
		if (!empty($saveAndRemainOnPage)) {
			header("Location: pages_boxes_edit.php?page=".$page_id->getValue()."&id=".$id->getValue());
		} else {
			header("Location: pages_boxes_select.php?page=".$page_id->getValue());
		}
		exit;
	}
} catch (Exception $e) {
