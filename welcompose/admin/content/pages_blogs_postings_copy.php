...
try {
	...
	$FORM->addElement('submit', 'save', gettext('Duplicate Posting'),
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('content/pages_blogs_postings_copy.html', WCOM_TEMPLATE_KEY);
		...
		// redirect
		header("Location: pages_blogs_postings_select.php?page=".$FORM->exportValue('page').$redirect_params);
		exit;
	}
} catch (Exception $e) {
	// clean the buffer
