...
try {
	...
	$_category = $PODCASTCATEGORY->selectBlogPodcastCategory(Base_Cnc::filterRequest($_REQUEST['id'], WCOM_REGEX_NUMERIC));
	...
	$category->addRule('callback', gettext('The given podcast category combination already exists'), 
		array(
			...
			'arguments' => array(array($subcategory->getValue(), $id->getValue()))
		)
	);
	...
	$FORM->addDataSource(new HTML_QuickForm2_DataSource_Array(array(
		'id' => Base_Cnc::ifsetor($_category['id'], null),
		'category' => Base_Cnc::ifsetor($_category['category'], null),
		'subcategory' => Base_Cnc::ifsetor($_category['subcategory'], null)
	)));
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('content/podcastcategories_edit.html', WCOM_TEMPLATE_KEY);
		...
		try {
			...
			// execute operation
			$PODCASTCATEGORY->updateBlogPodcastCategory($id->getValue(), $sqlData);
