...
try {
	...
	$FORM = $BASE->utility->loadQuickForm('blog_comment_status');
	...
	$name = $FORM->addElement('text', 'name', 
		array('id' => 'blog_comment_status_name', 'maxlength' => 255, 'class' => 'w300 validate'),
		array('label' => gettext('Name'))
		);
	$name->addRule('required', gettext('Please enter a name'));
	$name->addRule('regex', gettext('Please enter a valid name'), WCOM_REGEX_BLOG_COMMENT_STATUS_NAME);
	$name->addRule('callback', gettext('A blog comment status with the given name already exists'), 
		array(
			'callback' => array($BLOGCOMMENTSTATUS, 'testForUniqueName')
		)
	);
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('community/blogcommentstatuses_add.html', WCOM_TEMPLATE_KEY);
		...
		try {
			...
			// execute operation
			$BLOGCOMMENTSTATUS->addBlogCommentStatus($sqlData);
		...
		// redirect
		header("Location: blogcommentstatuses_add.php");
		exit;
	}
} catch (Exception $e) {
	// clean the buffer
