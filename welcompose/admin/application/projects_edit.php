...
try {
	...
	$FORM->addRule('name', gettext('A project with the given name already exists'),
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('application/projects_edit.html', OAK_TEMPLATE_KEY);
		...
		try {
			...
			// execute operation
			$PROJECT->updateProject($FORM->exportValue('id'), $sqlData);
