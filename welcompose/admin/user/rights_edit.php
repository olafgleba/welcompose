...
try {
	...
	// select right
	$right = $RIGHT->selectRight(Base_Cnc::filterRequest($_REQUEST['id'], WCOM_REGEX_NUMERIC));
	
	// start new HTML_QuickForm
	$FORM = $BASE->utility->loadQuickForm('right');
	...
	$name->addRule('regex', gettext('Please enter a valid name'), WCOM_REGEX_RIGHT_NAME);	
	$name->addRule('callback', gettext('A right with the given name already exists'), 
		array(
			'callback' => array($RIGHT, 'testForUniqueName'),
	...
	$description = $FORM->addElement('textarea', 'description', 
		array('id' => 'right_description', 'cols' => 3, 'rows' => 2, 'class' => 'w298h50'),
	...
	$FORM->addDataSource(new HTML_QuickForm2_DataSource_Array(array(
		'id' => Base_Cnc::ifsetor($right['id'], null),
		'name' => Base_Cnc::ifsetor($right['name'], null),
		'description' => Base_Cnc::ifsetor($right['description'], null)
	)));
	...
	if (!$FORM->validate()) {
		...
		$BASE->utility->smarty->display('user/rights_edit.html', WCOM_TEMPLATE_KEY);
		...
		try {
			...
			// execute operation
			$RIGHT->updateRight($id->getValue(), $sqlData);
		...
		// redirect
		header("Location: rights_select.php");
		exit;
	}
} catch (Exception $e) {
	// clean the buffer
